...
public class Exercises {
	...
	public boolean commonEnd(int[] a, int[] b) {
		...
		int lastElementOfA = a.length - 1;

		int lastElementOfB = b.length - 1;
		if (a[0] == b[0] || a[lastElementOfA] == b[lastElementOfB]) {
			return true;
		}
		return false;
	}
	...
	public int sum3(int[] nums) {
		int sumsOfAllThree = nums[0] + nums[1] + nums[2];
		return sumsOfAllThree;
	}
	...
	public int[] maxEnd3(int[] nums) {
		...
		if (nums[0] > nums[2]) {
			int [] changedArray = new int[] {nums[0], nums[0], nums[0]};
			return changedArray;
		}
		else if(nums[2] > nums [0]) {
			int [] changedArray = new int[] {nums[2], nums[2], nums[2]};
	...
	public int[] middleWay(int[] a, int[] b) {
		int [] middleElements = new int [] {a[1], b[1]};
	...
	public int sum13(int[] nums) {
		...
		for (int i = 0; i < nums.length; i++) {
			if(nums[i] != 13) {
				sumOfArray = nums[i] + sumOfArray;
			}
			else if (nums[i] == 13) {
				i++; //to skip an additional value since 13 is unlucky
			}
			else return 0;
		}
		return sumOfArray;
