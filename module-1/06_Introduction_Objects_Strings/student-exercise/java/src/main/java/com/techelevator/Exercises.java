...
public class Exercises {
	...
	public String middleTwo(String str) {
		int startingIndex = str.length() / 2;
		return str.substring(startingIndex - 1, startingIndex + 1);
	...
	public String twoChar(String str, int index) {
		if (index > str.length() - 2 || index < 0) { // what do i need here
			return str.substring(0, 2);
		} else
	...
	public String frontTimes(String str, int n) {
		String blank = "";
		String secondCase = str.substring(0);
		if (str.length() > 3) {
			for (int i = 0; i < n; i++) {
				blank = blank + str.substring(0, 3);
			}
			return blank;
		}
	...
	public int last2(String str) {
		...
		int ignoreLastTwo = str.length() - 2;
		int count = 0;
		if (ignoreLastTwo >= 0) {
			String endCompare = str.substring(ignoreLastTwo);
			for (int i = 0; i < ignoreLastTwo; i++) {
				if (str.substring(i, i + 2).equals(endCompare)) {
					count++;
				}
			}
		}
