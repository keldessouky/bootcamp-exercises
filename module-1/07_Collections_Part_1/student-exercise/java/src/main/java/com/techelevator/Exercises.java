...
public class Exercises {
	...
	public String[] list2Array(List<String> stringList) {
		...
		for (String s : stringList) {
			result[i++] = s;		//this can also be done with a vanilla for loop, done with for each
									//for example
		}
		return result;
	}
	...
	public boolean foundIntTwice(List<Integer> integerList, int intToFind) {
		...
		for (Integer duplicate : integerList) {

			if (intToFind == duplicate) {
				count++;
			}
	...
	** INTERVIEW QUESTION **
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		for (int addToResult: oneToTen) {
			results.add(addToResult);
		}
		for (int addToResult: elevenToTwenty) {
			results.add(addToResult);
		}
		for (int addToResult: twentyOneToThirty) {
			results.add(addToResult);
		}
