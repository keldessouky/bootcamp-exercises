...
SELECT name, region FROM country WHERE (continent = 'North America' OR continent = 'South America') AND region NOT IN ('Caribbean') ;
...
SELECT ((gnp * 1000000) / population) AS percapita FROM country WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT (2020 - indepyear) AS yearssinceindependent FROM country WHERE indepyear IS NOT NULL;
